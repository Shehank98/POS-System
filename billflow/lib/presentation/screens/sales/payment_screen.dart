...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  Future<void> _processPayment() async {
    ...
    // QR/Mobile → open QR payment screen first
    if (cart.paymentMethod == 'mobile') {
  ...
  Future<void> _handleQRPayment() async {
    ...
    // Record as 'qr' payment method
    ref.read(cartProvider.notifier).setPaymentMethod('qr');
    setState(() => _isProcessing = true);
    try {
      final updatedCart = ref.read(cartProvider);
      final txn =
          await ref.read(transactionsProvider.notifier).submitSale(updatedCart);
      if (mounted) _showSuccessDialog(txn);
    } on ApiException catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
