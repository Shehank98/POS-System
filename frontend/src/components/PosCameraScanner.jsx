 ...
 * Scanned items appear in a mini-cart below the viewfinder.
 * Tapping "Done" commits all items to the main cart via onDone().
 *
 * Props:
 *   onDone(items)  — called with [{product, quantity}] when user taps Done
 *   onClose()      — called to dismiss the modal (Cancel / X)
 */
export default function PosCameraScanner({ onDone, onClose }) {
  ...
  // Stable barcode handler — looks up product and adds to local list
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 flex flex-col">
      ...
      <div
        ...
        style={{ height: 220 }}
      >
        <div id="pos-multi-cam-view" className="w-full h-full" />

        {/* Scan flash overlay */}
        {flash && (
          <div className="absolute inset-0 flex flex-col items-center justify-center
                          pointer-events-none gap-2 bg-green-500/20">
            <div className="bg-green-500 text-white text-base font-bold
                            px-6 py-2.5 rounded-xl shadow-lg">
              ✓ Scanned!
            </div>
            {lastScanCode && (
              <p className="text-white font-mono text-xs bg-black/50 px-3 py-1
